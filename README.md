...
If you already run a Kubernetes cluster, pull the chart and install a
self-contained trial with its bundled preset:

```bash
helm pull oci://ghcr.io/experientiallabs/charts/experiential --version 0.1.0 --untar
helm install experiential ./experiential -f experiential/values-trial.yaml
```

Trial mode bundles Postgres, storage, and mail in-cluster for evaluation and
