...
/**
 * Per-(repeated prompt group, model) gateway usage rollups: which requests
 * came from the same prompt. Content-free — the groups are lineage digests,
...
export async function GET(request: Request, context: Context): Promise<Response> {
  try {
    ...
    const window = WINDOW_KEYS.includes(raw as ServingWindow)
      ...
      : undefined;
    const usage = await getDataSource().getUsageByPrompt(orgId, window);
