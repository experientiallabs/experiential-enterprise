-- ---------------------------------------------------------------------------
-- Gateway reservation/settlement seam: promotional-model cap + pre-verify
-- allowance (the product owner, 2026-08-28). Split from the config migration
-- (20260828130000_gateway_promotional_models) so this LAST redefinition of
-- gateway_start_attempt sorts after every other workstream's, and so the config
-- tables/helpers it depends on already exist. See that migration's header for
-- the full design. This file only recomposes gateway_start_attempt and
-- gateway_settle_attempt.
-- ---------------------------------------------------------------------------

-- ---------------------------------------------------------------------------
-- 7. Recompose gateway_start_attempt.
--
-- !!! SHARED FUNCTION BODY -- MERGE-TRAIN FLAG !!!
-- gateway_start_attempt is CREATE OR REPLACEd by several workstreams; the LAST
-- timestamp wins on a fresh migrate-all. This is the new LAST redefinition. It
-- carries EVERY existing money guard VERBATIM from
-- 20260826000000_decouple_spend_gate_from_login.sql:
--   * spend gate (decoupled from login)   P1025 (host lane) -- MODIFIED below
--   * RPM sliding window                   P1012
--   * TPM trailing settled-token window    P1022
--   * price-unknown fail-closed            P1013
...
-- and ADDS only:
--   (a) the per-(org, model) promo cap, resolved right after price-unknown and
--       wrapping the credit gates: promo-funded requests skip the credit gates
--       entirely (free); exhausted promo enters the credits path with P1030
--       (one-time notice) / P1031 (BYOK-only) transitions; and
--   (b) the pre-verify ALLOWANCE relaxation of the P1025 clause: an unverified
--       org may accrue up to the allowance of credit spend before P1025.
-- Anyone replacing this body later must carry ALL of the above.
-- ---------------------------------------------------------------------------
...
declare
  ...
  v_charge_worst pg_catalog.int8 := pg_catalog.coalesce(p_maximum_cost_micro_usd, 0);
...
begin
  ...
  if p_billing_source = 'host_managed' then
    ...
    if v_tpm is not null then
      ...
      select pg_catalog.coalesce(pg_catalog.sum(
          pg_catalog.coalesce(attempts.input_tokens, 0)
          + pg_catalog.coalesce(attempts.cached_input_tokens, 0)
          + pg_catalog.coalesce(attempts.output_tokens, 0)
          + pg_catalog.coalesce(attempts.reasoning_tokens, 0)), 0)
    ...
    v_percent_off := case when v_promo.is_promo then pg_catalog.coalesce(v_promo.percent_off, 0) else 0 end;
    if v_funding = 'credits' and v_percent_off > 0 then
      v_charge_worst := pg_catalog.round(
        pg_catalog.coalesce(p_maximum_cost_micro_usd, 0)::pg_catalog.numeric
        * (100 - v_percent_off) / 100
      )::pg_catalog.int8;
    else
      v_charge_worst := pg_catalog.coalesce(p_maximum_cost_micro_usd, 0);
    end if;
      ...
      if exists (
        ...
        select pg_catalog.coalesce(pg_catalog.sum(
            case when attempts.state = 'dispatched'
              ...
              else pg_catalog.coalesce(attempts.budget_settled_micro_usd, 0)
            end), 0)
  ...
  insert into public.gateway_attempts (
    ...
    case when v_funding = 'promo'
         then pg_catalog.coalesce(p_maximum_cost_micro_usd, 0) else 0 end,
...
-- ---------------------------------------------------------------------------
-- 8. Recompose gateway_settle_attempt so a promo-funded attempt settles into
--    the promo_* columns and NEVER draws org credits. Body is verbatim from
--    20260821200000_gateway_usage_metadata_errors.sql apart from the promo
--    branch at settlement. RETURNS type is unchanged, so CREATE OR REPLACE.

create or replace function public.gateway_settle_attempt(
...
begin
  ...
  if v_attempt.billing_source = 'host_managed' then
    v_settled := case
      when p_state = 'failed' or pg_catalog.coalesce(p_output_tokens, 0) = 0 then 0
      else pg_catalog.coalesce(v_cost, 0)
    end;
  else
    -- Never charged; the conservative attributed value mirrors WMO's ledger.
    v_settled := pg_catalog.coalesce(v_cost, v_attempt.budget_reserved_micro_usd);
  end if;
...
-- CREATE OR REPLACE preserves the existing grants on this signature (revoked
-- from public/anon/authenticated; executable by service_role).
