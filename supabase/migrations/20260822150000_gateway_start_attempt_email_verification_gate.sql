...
-- must carry ALL of:
--   * email-verification spend gate           P1025 (host lane, added here)
...
-- pgTAP pins the COMPOSED function (budget enforcement, runtime, and the new
-- email-verification-gate suites); explabs/gateway/e2e_test.py pins it
-- end-to-end. Extend those when touching this body.
--
-- Changes vs 20260822130000 (everything else is verbatim):
--   The email-verification spend gate (P1025 org_owner_unverified). An org
--   whose owner -- its founding 'admin' membership -- has NOT confirmed their
--   email (auth.users.email_confirmed_at is null) may not draw PLATFORM
--   credits. This is the money half of instant signup (POST
--   /api/signup/instant): that endpoint provisions a real org with a usable
--   xpl_ key so a founder can wire the gateway and land traces as telemetry
--   IMMEDIATELY, but the credit grant stays locked until they click the
--   verification email (which sets email_confirmed_at, opening the gate with no
--   further action). BYOK is unaffected: this block is host-lane only. The gate
--   blocks ONLY when a present admin membership is unverified, so an org with
--   no membership (fixtures, and every existing host-lane pgTAP seed) is never
--   gated. It fires FIRST inside the host block -- right after the org money
--   lock, before the balance/cap/budget checks -- because an unverified org has
--   no business reaching any spend computation. It runs under the same
--   organizations row lock as the caps. Real (browser) signups are auto-
--   confirmed (auth email confirmations are disabled), so email_confirmed_at is
--   already set for every paying customer; only instant-signup orgs are null.
-- ---------------------------------------------------------------------------
...
begin
  ...
  if p_billing_source = 'host_managed' then
    ...
    perform 1 from public.organizations orgs
     ...
     for update;
    -- Email-verification spend gate (money half of instant signup). An org
    -- whose founding admin has not confirmed their email cannot draw platform
    -- credits; everything else (BYOK, trace uploads, wiring the gateway,
    -- dashboard) is unaffected. Fires FIRST in the host block, before any
    -- spend computation, and only when a present admin membership is
    -- unverified -- so a membership-less fixture org is never gated. Opens
    -- automatically once the founder clicks the verification email
    -- (email_confirmed_at is set). BYOK skips this whole block.
    if exists (
      select 1
