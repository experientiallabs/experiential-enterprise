...
select has_function(
  'public',
  'invoke_balance_fetch',
  'the balance-fetch job body exists as an invokable function'
);

select is(
  (select count(*) from cron.job
   where jobname = 'account-balance-fetch' and schedule = '30 4 * * *'),
  1::bigint,
  'the account-balance sweep is scheduled nightly at 04:30 UTC'
);
...
where name in ('balance_fetch_url', 'cron_secret');
...
select vault.create_secret(
  'https://app.example.test/api/internal/balance-fetch',
  'balance_fetch_url'
);
...
select isnt_empty(
  ...
  where url = 'https://app.example.test/api/internal/balance-fetch'
  $$,
  'the request targets the Vault-configured balance-fetch URL'
);
